...
function updateMiniCalendar(calendarID, month, year) {
...
function createMiniCalendar(label) {
	...
	var daycode = (year + Math.floor((year - 1) / 4) - Math.floor((year - 1) / 100) + Math.floor((year - 1) / 400) ) % 7;
	...
	if (month == 0) {
		jQuery($(tr[0]).append('<th><button type ="button" onclick="updateMiniCalendar(' + calendarCurrentID + ', '+ 11 + ','+ (year - 1)  +')">le</button></th>'));
	}else{
		jQuery($(tr[0]).append('<th><button type ="button" onclick="updateMiniCalendar(' + calendarCurrentID + ', '+ (month - 1) + ','+ year +')">le</button></th>'));
	}
	jQuery($(tr[0]).append('<th colspan="5">'+ monthName[month] + ' ' + year + '</th>'));

	if (month == 11) {
		jQuery($(tr[0]).append('<th><button type ="button" onclick="updateMiniCalendar(' + calendarCurrentID + ', '+ 0 + ','+ (year + 1)  +')">ri</button></th>'));
	}else{
		jQuery($(tr[0]).append('<th><button type ="button" onclick="updateMiniCalendar(' + calendarCurrentID + ', '+ (month + 1) + ','+ year +')">ri</button></th>'));
	}
	...
	//jQuery($(minicalendar).append('<label for="' + label + '' + calendarCurrentID '">' + label + '</label>', table, '<input type="text" class="calendar-form-control" id="' + label + '' + calendarCurrentID '">'));
